...
GaussPt=8;
 ...
 [x,w]=GaussLegendre(GaussPt);GaussXW =[x w]';
s=-0.5;%x(8);%
MChau=0;
% 以下是程序部分
...
NodeNo=find( IndexUPknow(:,end)==2 |IndexUPknow(:,end)==3);
CorPoint=[];
for iN=1:numel(NodeNo)
    ...
    CorPoint=[CorPoint; Nodei];
end
 CP=zeros(size(CorPoint,1),2);
for i=1:size(CorPoint,1)
    CP(i,1)=CorPoint(i);
    if IndexUPknow(CorPoint(i),3)==0
         if  ~isempty(find(CommonNodes(:,1)==CorPoint(i)))
              CP(end+1,1)=CommonNodes(find(CommonNodes(:,1)==CorPoint(i)),2);
         else  
             CP(end+1,1)=CommonNodes(find(CommonNodes(:,2)==CorPoint(i)),1);
         end
        continue
    end   
       CP(i,2)=1;  %CP(i,2)=1 边界角点 ；0 界面角点 
end
%CP=[8 0;11 0;19 0;23 0;25 0;26 0;4 1]; 
    ...
    for iN=1:size(NodeAi,1)   
      ...
      if ~isempty(find(CP(1:end,1)==np))     
        ...
        disp(sprintf('Area=%d        iN=%d',iA,NodeAi(iN)))
         ...
         RIndex=2*np+[-1 0];
         [Hnp Gnp VFnp Cnp]=HGFA(np,ElementAi,IntMethod);
         H(RIndex,:)=H(RIndex,:)+Hnp+Cnp;
...
for i=1:size(CP,1)
    if CP(i,2)==0
        RIndex=2*CP(i,1)+[-1 0];
        ENo=find(Element(:,3)==CP(i,1));
        Nodei=CP(i,1);
        Nodej=Element(ENo,4);
        Nodek=size(Node,1)+1;
        CorElement=ENo;
        CorNode=1/2*[(1-s) (1+s)]*[Node(Nodei,2:3);Node(Nodej,2:3)];
        Node(end+1,:)=[Nodek CorNode];       
        np=Nodek;
        iA=Element(ENo,1);
        MatiA=Area(iA,2);
        ElementAi=Element(find(Element(:,1)==iA),:);
        [Hnp Gnp VFnp]=HGFAC(np,ElementAi,IntMethod,CorElement);
        Node(end,:)=[];
        H(RIndex,:)=Hnp;
        G(RIndex,:)=Gnp;
        VF(RIndex,:)=VFnp;
...
for iN=1:size(CommonNodes,1)
    ...
    IndexUPknow(iN1,9)=IndexUPknow(iN1,9)+2; %所有位移连续条件给从节点
end
%施加剪应力正应力相等条件iDOF代表前后
for iCE=1:size(CommonElement,1)
    for iN=1:2
...
%            (IndexUPknow(iNM,end) < 4 & IndexUPknow(iNS,end) < 4)
            IndexUPknow(iNM,9)=IndexUPknow(iNM,9)+1;
            IndexUPknow(iNS,9)=IndexUPknow(iNS,9)+1;
        else
            for iDOF=1:2        %所有的连续条件首先满足方程少的点
%                 以前的程序
...
%                 end
                Rows=[iNM iNS];
                [CMin IMin] = min(IndexUPknow(Rows,9));
                RMin=Rows(IMin);
                IndexUPknow(RMin,9)=IndexUPknow(RMin,9)+1;
            end            
        end
    end
end
...
NodeNo=find( IndexUPknow(:,9)==2 | IndexUPknow(:,9)==3);
for iN=1:numel(NodeNo)
    Nodei=NodeNo(iN);
    Eb=find(Element(:,4)==Nodei);
    Ea=find(Element(:,3)==Nodei);
    ...
    if IndexUPknow(Nodei,end)==3 & norm(N1-N2)<1e-3   % ~isempty(find(Nodei==CP(:,1)))
    ...
    if  norm(N1-N2)<1e-3   %&  isempty(find(Nodei==CP(:,1)))%两单元共线，特殊的角点约束
        MN=(N1+N2)/2;
    ...
    elseif MChau==1%加另一个角点约束            
        ...
        F(end)=[];
        %continue
        if CP(find(CP(:,1)==Nodei),2)==1 %& IndexUPknow(Nodei,end)==66 %边界角点
           continue
        end
%        if  Nodei==6
%           Nodei=21;
